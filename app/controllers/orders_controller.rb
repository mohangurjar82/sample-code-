class OrdersController < ApplicationController
  ...
  def new
    ...
    @order = current_user.orders.build(credit_card: CreditCard.new)
  end
  ...
  def create
    ...
    @order = current_user.orders.build(credit_card: CreditCard.new(cc_params), product_ids: session[:product_ids])
